...
import { BankingMethodCard } from '@/pages/user/wallet/components/banking-method-card';
import type { IBankingMethod } from '@/interfaces/wallet/wallet.interface';
import { CircleCheckBig, Clock, Download, FileText, Plus, TrendingUpIcon, WalletIcon } from 'lucide-react';
                        ...
                        <>
                            <Card className='bg-linear-to-br from-[#390085] to-[#4a0a9e] dark:from-[#8b5cf6] dark:to-[#7c3aed] rounded-xl p-6 text-white gap-2'>
                                ...
                                <CardContent className='p-0'>
                                    <p className='font-semibold text-3xl'>{formatCurrency(walletBalance.data?.data?.wallet?.balance, 'USD')}</p>
                                </CardContent>
                                <CardFooter className='p-0'>
                                    <Button size="sm" className='flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg font-medium text-[13px] transition-colors'><Download /> Withdraw Funds</Button>
                                </CardFooter>
                            </Card>
                            ...
                            <Card className='bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 gap-2'>
                                ...
                                <CardContent className='p-0'>
                                    <p className='font-semibold text-3xl text-green-600 dark:text-green-400'>{formatCurrency(walletBalance.data?.data?.wallet?.availableBalance, 'USD')}</p>
                                    <p className='font-normal text-[12px] text-gray-500 dark:text-gray-400 mt-1'>Ready to withdraw</p>
                                </CardContent>
                            </Card>

                            <Card className='bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 gap-2'>
                                <CardHeader className='p-0'>
                                    <CardTitle className='flex items-center font-normal text-[13px] text-gray-500 dark:text-gray-400'><Clock className='size-5 mr-2' />Pending</CardTitle>
                                </CardHeader>
                                <CardContent className='p-0'>
                                    <p className='font-semibold text-3xl text-red-600 dark:text-red-400'>{formatCurrency(walletBalance.data?.data?.wallet?.holdBalance, 'USD')}</p>
                                    <p className='font-normal text-[12px] text-gray-500 dark:text-gray-400 mt-1'>Awaiting approval</p>
                                </CardContent>
                            </Card>

                            <Card className='bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 gap-2'>
                                <CardHeader className='p-0'>
                                    <CardTitle className='flex items-center font-normal text-[13px] text-gray-500 dark:text-gray-400'><TrendingUpIcon className='size-5 mr-2' />Total Earned</CardTitle>
                                </CardHeader>
                                <CardContent className='p-0'>
                                    <p className='font-semibold text-3xl'>{formatCurrency(walletBalance.data?.data?.wallet?.totalEarned, 'USD')}</p>
                    ...
                    {transactions.isLoading ? (
                        ...
                        <div className="space-y-3">
                            {transactions.data?.data?.items?.map((tx: ITransaction) => (
                                <TransactionCard key={tx._id} transaction={tx} role="contractor" />
                            ))}
                        </div>
                    )}

                    {transactions?.data?.data?.pagination?.pages > 1 && (
                        <AppPagination
