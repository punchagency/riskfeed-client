    ...
    useEffect(() => {
        if (property) {
            form.reset({
                ...
                purchaseDate: propertyData?.purchaseDate ? new Date(propertyData?.purchaseDate).toISOString().split('T')[0] : '',
                purchasePrice: propertyData?.purchasePrice?.toString() || '',
                estimatedValue: propertyData?.estimatedValue?.toString() || '',
                currentEstimatedValue: propertyData?.currentEstimatedValue?.toString() || '',
                annualPropertyTax: propertyData?.annualPropertyTax?.toString() || '',
                annualInsurance: propertyData?.annualInsurance?.toString() || '',
                annualMaintenanceCosts: propertyData?.annualMaintenanceCosts?.toString() || '',
                squareFeet: propertyData?.squareFeet?.toString() || '',
                yearBuilt: propertyData?.yearBuilt?.toString() || '',
                noOfBedrooms: propertyData?.noOfBedrooms?.toString() || '',
                noOfBathrooms: propertyData?.noOfBathrooms?.toString() || '',
    ...
    const onSubmit = (data: z.infer<typeof propertySchema>) => {
        ...
        if (propertyImages.length > 0) {
            ...
            if (imagesToRemove.length > 0) {
                imagesToRemove.forEach(img => formData.append('imagesToRemove[]', img));
            }
            propertyImages.forEach(image => formData.append('propertyImages', image));

            updateProperty.mutate({ id: propertyId, data: formData as unknown as UpdatePropertyDto }, {
